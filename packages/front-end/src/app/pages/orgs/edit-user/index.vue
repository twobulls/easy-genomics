...
<template>
  ...
  <div class="mb-4">
    <EGUserOrgAdminToggle :key="useOrgsStore().selectedUser?.UserId" :user="useOrgsStore().selectedUser"
      @update-user="updateSelectedUser($event)" />
  </div>

  <EGSearchInput v-if="hasNoData" @input-event="updateSearchOutput" placeholder="Search All Labs"
    class="my-6 w-[408px]" />

  <EGEmptyDataCTA v-if="hasNoData" message="There are no labs in your Organization"
    :button-action="() => $router.push({ path: '/labs/new' })" button-label="Create a Lab" />

  <EGTable v-if="!hasNoData" :table-data="filteredTableData" :columns="tableColumns" :show-pagination="!isLoading"
    ...
    <template #actions-data="{ row }">
      ...
      <EGButton :loading="useUiStore().isRequestPending" v-else-if="row.access" @click="
        ...
        " label="Grant access" variant="secondary" size="sm" />
