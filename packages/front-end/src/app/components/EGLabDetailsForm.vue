<script setup lang="ts">
  ...
  async function onSubmit(event: FormSubmitEvent<LabDetails>) {
    if (formMode.value === LabDetailsFormModeEnum.enum.ReadOnly) return;

    try {
      const formParseResult = LabDetailsSchema.safeParse(event.data);
  ...
  async function handleCreateLab(labDetails: LabDetails) {
  ...
  async function handleUpdateLabDetails(labDetails: LabDetails) {
...
<template>
  ...
  <UForm v-else :validate="validate" :schema="LabDetailsSchema" :state="state" @submit="onSubmit">
    <EGCard>
      ...
      <EGFormGroup v-if="!isS3BucketFieldHidden" label="S3 Bucket" name="S3Bucket">
